...
import { SITE_ORIGIN } from '../seo/schema'
...
export function useSeo({
  ...
  useEffect(() => {
    const full = /VEITH/i.test(title) ? title : `${title} | ${SUFFIX}`
    document.title = full
    ...
    const card = image ? `${SITE_ORIGIN}/img/${image}-1280.webp` : `${SITE_ORIGIN}/og-default.jpg`
    ...
    meta('meta[name="twitter:image"]', 'name', 'twitter:image', card)
  ...
  }, [title, description, path, image, type, serialized, robots])
}
